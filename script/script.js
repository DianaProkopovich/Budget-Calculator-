const generateId = () => `me${Math.round(Math.random() * 1e8).toString(16)}`
...
// в стрелочной функции скобки не обязательны если аргумент один
...
// редус запускается столько раз, сколько элементов в массиве,
// возвращенное значение функции будет записано в результат ( аккумулятор)
...
const init = () => {
    ...
    updateBalance();
    // dbOperation.forEach((item, index) => {
